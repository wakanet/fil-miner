...
RUST_LOG=info RUST_BACKTRACE=1 NETIP=$netip GOMAXPROCS=$cpu_num ./lotus-worker --worker-repo=$worker_repo --miner-repo=$miner_repo --storage-repo=$storage_repo run --id-file="$worker_id_file" --max-tasks=12 --parallel-pledge=6 --parallel-precommit1=6 --parallel-precommit2=2 --parallel-commit=1 &
pid=$!
