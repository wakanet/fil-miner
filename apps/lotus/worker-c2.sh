...
export worker_id_file="~/.lotusworker/worker-c2.id" # need open for testing
...
RUST_LOG=info RUST_BACKTRACE=1 NETIP=$netip ./lotus-worker --worker-repo=$worker_repo --miner-repo=$miner_repo --storage-repo=$storage_repo run --id-file="$worker_id_file" --listen-addr="$netip:1283" --parallel-pledge=0 --parallel-precommit1=0 --parallel-precommit2=0 --parallel-commit=0 --commit2-srv=true &
pid=$!
