...
RUST_LOG=info RUST_BACKTRACE=1 NETIP=$netip ./lotus-worker --worker-repo=$worker_repo --miner-repo=$miner_repo --storage-repo=$storage_repo run --id-file="$worker_id_file" --max-tasks=28 --parallel-pledge=28 --parallel-precommit1=28 --parallel-precommit2=2 --parallel-commit=0 &
pid=$!
